...
class Settings(BaseSettings):
    ...
    # Path to monitor (Nginx and SSH)
    NGINX_LOG_PATH: str = "/var/log/nginx/access.log"
    SSH_LOG_PATH: str = "/var/log/auth.log"

    BOTNET_THRESHOLD: int = 20
    TIME_WINDOW: int = 30  # seconds

    SSH_MAX_ATTEMPTS: int = 5
    SSH_TIME_WINDOW: int = 600  # seconds (10 mins)
...
if settings.MOCK_MODE:
    if settings.NGINX_LOG_PATH == "/var/log/nginx/access.log":
        settings.NGINX_LOG_PATH = "./test_access.log"
    if settings.SSH_LOG_PATH == "/var/log/auth.log":
        settings.SSH_LOG_PATH = "./test_auth.log"
